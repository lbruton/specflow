...
- [ ] {N+5}. Security Review — Codacy SRM scan
  ...
  - _Leverage: `/codacy-resolve` skill, `mcp__codacy__codacy_search_repository_srm_items` (SRM/security — the gate tool), `mcp__codacy__codacy_list_repository_issues` (supplementary code-quality check, NOT SRM — do not substitute for the SRM call) filtered to changed files_
...
- [ ] {N+7}. Cross-Model Peer Review
  - **Recommended Agent:** Claude (orchestrator) — dispatches review to Codex, CodeRabbit, or pr-review-toolkit
  - File: (no file changes — review only, fixes happen via loop-back if needed)
  - **Review dispatch chain (try in order, use first that works):**
    ...
    2. **CodeRabbit (`coderabbit:review`)** — if Codex dispatch fails or is unavailable, invoke the `coderabbit:review` skill via the Skill tool. Catches logic bugs, architecture issues, test gaps, and PR-level AI review findings that often appear in post-PR automated scans — running it pre-PR prevents surprise review threads.
    3. **Built-in code review (`pr-review-toolkit:review-pr`)** — if both Codex and CodeRabbit fail, fall back to the multi-agent code review skill. Runs spec-compliance, quality, silent-failure, and type-design analyzers.
    4. **Mark `[!]` BLOCKED** — only if ALL options above fail. Present to user for manual review decision.
  - **Self-detection guard:** IF running from Codex itself (check `$CODEX_SESSION` env var) -> skip Codex dispatch, go directly to option 2 (`coderabbit:review`).
  ...
  - Purpose: Cross-model peer review catches blind spots a single AI might miss; CodeRabbit as step 2 mirrors what automated post-PR scans will flag, so findings are resolved before the PR is opened rather than after
  - _Leverage: `codex:codex-rescue` agent type (primary), `coderabbit:review` skill (step 2), `pr-review-toolkit:review-pr` skill (fallback), branch git diff_
  - _Requirements: All_
  - _Prompt: Implement the task for spec {{spec-name}}, first run mcp__specflow__spec-workflow-guide to get the workflow guide then implement the task: Role: Code Review Coordinator | Task: Run a cross-model peer review using this dispatch chain (try in order, use first that works): (1) If NOT running from Codex ($CODEX_SESSION not set): attempt to dispatch via Agent tool with subagent_type "codex:codex-rescue" and run_in_background true. The review prompt: "Review the git branch diff. Run `git diff $(git merge-base HEAD main || git merge-base HEAD dev || echo main)...HEAD`. Check each file for: correctness, CLAUDE.md compliance, security, pattern consistency, edge cases. Report by severity: Critical / Important / Minor with file:line references." (2) If Codex dispatch fails OR you are running from Codex: invoke the coderabbit:review skill via the Skill tool. CodeRabbit catches logic bugs, architecture issues, and test gaps — running it here mirrors what the automated post-PR CodeRabbit scan will find, so findings are resolved pre-PR. (3) If CodeRabbit also fails: fall back to the built-in pr-review-toolkit:review-pr skill — invoke it via the Skill tool. This runs spec-compliance, code quality, silent-failure, and type-design analyzers. (4) If ALL fail: mark this task [!] BLOCKED with reason. After review completes (any path): fix Critical and Important findings by looping back to implementation. Minor findings are advisory. | Restrictions: Do NOT check `command -v codex` — the CLI binary is irrelevant; the agent type availability is what matters. NEVER mark [x] if the review was fully skipped — use [!]. Do not dismiss findings without investigating. | Success: Review completed via Codex OR CodeRabbit OR pr-review-toolkit with Critical/Important issues addressed, OR [!] BLOCKED with documented reason. BLOCKING: You MUST call `mcp__specflow__log-implementation` with review findings before marking [x] or [!]. Do NOT mark [x] or [!] until the tool call succeeds._
